    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        joinEventBtn.setOnClickListener {
            ...
            if (eventIdEdTe.length() != 6) {
                Toast.makeText(
                    activity,
                    "Du musst eine 6-stellige Zahl eingeben",
                    Toast.LENGTH_LONG
                ).show()
            } else {
                            ...
                            db.collection("answers")
                                ...
                                .addOnCompleteListener { documents ->
                                    if (documents.result.size() == 0) {
                                        ...
                                        val data1 = db.collection("events")
                                            .document(eventIdEdTe.text.toString())
                                        data1
                                            .update("votes", FieldValue.arrayUnion("$uid"))
                                            .addOnSuccessListener {
                                                Log.i(
                                                    TAG,
                                                    "Antwort wurde hinzugefügt"
                                                )
                                            }
                                            .addOnFailureListener {
                                                Log.i(
                                                    TAG,
                                                    "Antwort konnte nicht hinzugefügt werden"
                                                )
                                            }
                                ...
                                .addOnFailureListener { exeption ->
                                    Log.e(
                                        TAG,
                                        "Fehler bei Überprüfung, ob User schon bereits abgestimmt hat",
                                        exeption
                                    )
                                }
                        }
